...
class TokenManager:
    ...
    def __init__(self, token: str):
        ...
        self._jwt_expires_at = 0  # Unix timestamp when JWT expires
    ...
    def is_token_expired(self) -> bool:
        ...
        return time.time() >= self._jwt_expires_at
    ...
    def set_jwt_token(self, jwt_token: str):
        ...
        # Decode token to get expiration time (without signature verification)
        decoded = jwt.decode(jwt_token, options={"verify_signature": False})
        self._jwt_expires_at = decoded.get("exp")
...
class HttpxClient:
    ...
    def __init__(
            self, token_manager: TokenManager, url: str
    ):
    ...
    def _auth_headers(self):
        return (
            {"Authorization": f"Bearer {self._token_manager.jwt_token}"}
    ...
    async def _exec_request(
        ...
        async with httpx.AsyncClient(
                http2=True, timeout=5, headers=self._auth_headers
        ) as client:
